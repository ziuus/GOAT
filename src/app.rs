        ...
        match name.as_str() {
            ...
            "/designer" => {
                ...
                match subcmd {
                    "status" => {
                        self.push_log(format!(
                            "[DESIGNER] Deep designer workflow active. Handoffs available."
                        ));
                    }
                    _ => {
                        self.push_log(format!(
                            "[DESIGNER] Deep design review task scheduled for {}",
                            subcmd
                        ));
                    }
                }
            ...
            | "@rollback" => {
                ...
                let target = parts.get(1).copied().unwrap_or("").trim();
                let agent = match crate::agents::operator::OperatorAgent::new() {
                    Ok(a) => a,
                    Err(e) => {
                        self.push_log(format!("[OPERATOR] Init error: {}", e));
                        return true;
                    }
                };
                ...
                match subcmd {
                    ...
                    _ => self.push_log(format!("[OPERATOR] Unknown subcmd: {}", subcmd)),
                }
                true
            }
            ...
            "/socializer" => {
                ...
                match subcmd {
                    "status" => {
                        self.push_log(format!(
                            "[SOCIALIZER] New socializer agent active. Deep workflows incoming."
                        ));
                    }
                    _ => {
                        self.push_log(format!(
                            "[SOCIALIZER] Unknown or unimplemented command: {}",
                            subcmd
                        ));
                    }
                }
